...
export const rules = {
   ...
   * 电话号正则，包含手机号、固话号、400电话
   */
  phone: /^(0|86|17951)?(13[0-9]|14[579]|15[012356789]|166|17[1235678]|18[0-9]|19[189])\s{0,1}[0-9]{4}\s{0,1}[0-9]{4}$|^(0[0-9]{2,3}-?)?[0-9]{7,8}$|^400-?\d{3}-?\d{4}$/,
  /**
   * 手机号正则
   */
  mobilePhone: /^(0|86|17951)?(13[0-9]|14[579]|15[012356789]|166|17[1235678]|18[0-9]|19[189])\s{0,1}[0-9]{4}\s{0,1}[0-9]{4}$/,
  /**
   * 固话正则，包含400电话
   */
...
export const getRules = (required?: boolean, originRules?: RulePropsType) => {
  const rulesObj: any = []
  if (required) {
    rulesObj.push(_rules.required)
  }
  originRules &&
    originRules.forEach((item: any) => {
      if (kindOf(item) === 'string') {
        const rule = _rules[item]
        rulesObj.push(rule)
      } else if (item.type) {
        const rule = _rules[item.type]
        const customMessage = {
